...
ИЗМЕНЕНО (нумерация устройств): каждому перенесённому устройству
присваивается device_number по порядку его исходного id внутри
конкретного пользователя (первое устройство пользователя -> 1, второе
-> 2, и т.д.), а User.next_device_number выставляется в
(количество устройств пользователя + 1). Это нужно, чтобы после миграции
новое устройство, добавленное через бота, получило правильный следующий
номер и не пересеклось с уже перенесёнными (см.
app/db/models/user.py::next_device_number,
app/providers/xui/client.py::remote_id_for).

Старые per-device VLESS-клиенты в панели НЕ переименовываются — они
переносятся как есть (client_remote_id=vpn_email старого формата,
"user_{tid}_dev{legacy_device_id}"), без сетевых вызовов к панели.
Новый формат remote_id ("{tid}_device_{N}") будет применяться только к
устройствам, добавленным ПОСЛЕ этой миграции.
"""
...
def _read_legacy_devices(conn: sqlite3.Connection) -> list[dict]:
    rows = conn.execute(
        "SELECT id, user_id, device_name, vpn_email, vpn_uuid FROM devices ORDER BY id ASC"
    ).fetchall()
    return [dict(r) for r in rows]
...
def run(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        # Счётчик следующего device_number на пользователя (в рамках ЭТОГО
        # прогона миграции) — растёт по порядку исходного legacy id.
        next_number: dict[int, int] = defaultdict(lambda: 1)
        ...
        for row in legacy_devices:
            ...
            device_number = next_number[owner.telegram_id]
            next_number[owner.telegram_id] = device_number + 1

            device = Device(user_id=owner.id, device_name=row["device_name"], device_number=device_number)
            if legacy_uuid:
        ...
        # Выставляем next_device_number пользователям = (кол-во перенесённых
        # устройств + 1), чтобы следующее добавленное через бота устройство
        # не пересеклось по номеру с уже перенесёнными.
        for telegram_id, number in next_number.items():
            by_telegram_id[telegram_id].next_device_number = number
