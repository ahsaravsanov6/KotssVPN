...
def test_build_body_two_devices_produce_different_uuids(sample_server, sample_device):
    second_device = Device(id=2, user_id=sample_device.user_id, device_name="Android", device_number=2, uuid="aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee")
...
def test_build_body_single_device_matches_per_device_slice(sample_server, sample_device):
    """
    Ключевая проверка новой per-device подписки: тело, построенное ТОЛЬКО
    для одного устройства (как это делает
    SubscriptionService.build_device_subscription), идентично "срезу"
    полного тела для всех устройств по этому же device — т.е. переход на
    персональные ссылки не меняет формат самих vless-записей.
    """
    second_device = Device(id=2, user_id=sample_device.user_id, device_name="Android", device_number=2, uuid="aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee")

    full_body = SubscriptionGenerator.build_body([
        (sample_device, [sample_server]),
        (second_device, [sample_server]),
    ])
    device_only_body = SubscriptionGenerator.build_body([(sample_device, [sample_server])])

    full_lines = base64.b64decode(full_body).decode("utf-8").strip().split("\n")
    device_only_lines = base64.b64decode(device_only_body).decode("utf-8").strip().split("\n")

    assert device_only_lines[0] in full_lines
