...
def test_same_device_different_servers_same_uuid(sample_server, sample_device):
    """Инвариант: UUID устройства одинаков во всех сгенерированных для
    него ссылках, различаться должны только серверные параметры."""
    other = Server(
    ...
    assert uri1.split("@")[0] == uri2.split("@")[0]  # "vless://<uuid>" одинаков
...
def test_different_devices_same_server_different_uuid(sample_server, sample_device):
    """Инвариант в другую сторону: два разных устройства одного
    пользователя на одном сервере получают РАЗНЫЕ UUID (это и есть
    техническая основа лимита устройств — каждый ключ свой)."""
    from app.db.models.device import Device

    other_device = Device(id=2, user_id=sample_device.user_id, device_name="Android", uuid="99999999-8888-7777-6666-555555555555")
