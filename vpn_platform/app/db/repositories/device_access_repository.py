...
Изменено: server_id — обычная строка (id сервера из servers.yaml), а не
внешний ключ, поэтому servers_for_device() (требовавший ORM-связь
row.server) отсюда убран — резолв server_id -> ServerConfig теперь
явно делает вызывающий сервисный слой (SubscriptionService) через
ServerManager.get_by_id(), см. его докстринг о причинах.
"""
