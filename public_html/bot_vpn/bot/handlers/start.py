...
Архитектура навигации:
    Главное меню (Reply): 🔑 Мой VPN | 💳 Подписка | ⚙️ Ещё
    ⚙️ Ещё (Inline):       👤 Личный кабинет, 👥 Пригласить друга,
                           📢 Новости, 🆘 Поддержка, ℹ️ О нас

Личный кабинет здесь больше не показывает статус подписки — это раздел
профиля и устройств. Статус подписки и её покупка/продление живут в
bot/handlers/subscription.py (единый источник правды для этих данных).
"""
...
# Ссылка на новостной канал — задайте в .env или замените здесь
...
async def cmd_start(message: Message) -> None:
    ...
    # Обработка реферальной ссылки (/start ref_<telegram_id>)
...
async def handle_more_menu(message: Message) -> None:
    """Открывает свёрнутые редкие разделы: личный кабинет, рефералка, новости и т.д."""
    await message.answer(
        text="⚙️ <b>Ещё</b>\n\nВыберите раздел:",
...
async def callback_more_menu(callback: CallbackQuery) -> None:
    """Возврат в раздел 'Ещё' по инлайн-кнопке."""
    await callback.answer()
...
# ── Личный кабинет (профиль + устройства, без статуса подписки) ───────────────

@router.callback_query(F.data == "menu:account")
async def callback_account_menu(callback: CallbackQuery) -> None:
    """Открывает личный кабинет по инлайн-кнопке (из 'Ещё' или из подписки)."""
    await callback.answer()
